...
def test_success(conn: dbver.Connection, lock_mode: dbver.LockMode) -> None:
    def pool() -> ContextManager[dbver.Connection]:
        return contextlib.nullcontext(conn)

    conn.cursor().execute("CREATE TABLE x (x INT PRIMARY KEY)")
    check_in_transaction(conn, False)
    with dbver.begin_pool(pool, lock_mode) as inner:
        check_in_transaction(inner, True)
        inner.cursor().execute("INSERT INTO x (x) VALUES (1)")
    check_in_transaction(conn, False)
